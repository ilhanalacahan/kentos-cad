...
- **Sıcak yollar (S1):** kenet, seçme, etiket, tutamaç, hayaletler ve budama önizlemesi JSON tablosundan geçmez. Belgenin kopyasını tutan bir geometri deposuna (`Store`) tipli, `Float64Array` giriş-çıkışlı toplu sorgular yapılır. Ayrıntısı aşağıda “Geometri deposu” başlığındadır.
...
- **Aynı makinede ölçüm (bulut, Node 22, imleç başına):** `parsel-50k`'da seçme yakın görünümde 10,2 → 0,008 ms, genel görünümde 9,9 → 0,05 ms; kenet yakında 10,1 → 0,01 ms, genelde 17,4 → 1,2 ms. Uygulamadaki etkileşim ölçümü S1'in sonunda alınır (`docs/perf`).

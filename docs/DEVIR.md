...
- **WASM paketi:** 898 018 bayt, gzip 303 940 bayt (sağlam kararlar R1–R4 ve paketli dönüşümle; önce 883 626 / 297 902; DXF yazıcısı değiştirmedi). Başlangıç sınırı kullanıcı kararıyla 300'den 350 KB gzip'e yükseltildi (24 Eylül, ADR 0005 “Değişiklikler”); işlev adları pakette kalır. Her çekirdek diliminden sonra boyut ADR 0008'e yazılır.
- **Son doğrulama** (`main`, DXF ölçüsü, 24 Eylül, bulut konteyneri): `pnpm typecheck` temiz; `pnpm test` 724 test geçti, 7 atlandı (fixture kaydedicileri ve istekle çalışan dönüşüm ölçümü); `pnpm build` başarılı (dışa aktarma penceresi ayrı parça, 6,5 kB); `pnpm rust:test` temiz (287 test, clippy `-D warnings`; veritabanı testleri sır olmadığı için atlandı); `cargo fmt --all -- --check` temiz; `pnpm e2e` 134 denetim geçti, düşen üçü bu konteynerde düşen WebGPU denetimleri (§5; şeritten önceki `6505a6e` de aynı piksel sayılarıyla düşüyor). Alt ajan ayrıca örnek bir dosyayı ezdxf 1.4 ile denetledi (hata yok); ölçülü dosya da hatasız ve düzeltmesiz okunur, ezdxf'in yeniden çizdiği değerler KentOS'unkiyle aynıdır. Tam takımda `ellipse.test.ts`'in yoğun örnekleme denetimi makine yüklüyken bir kez 5 sn sınırını aştı; kendi süresi 30 sn yapıldı (`67c663d`).
- **Ölçüm:**

...
- **WASM paketi:** 1 128 063 bayt, gzip 394 938 bayt (stil derleyicisiyle; ifade dilinden sonra 1 008 667 / 349 550). Başlangıç sınırı kullanıcı kararlarıyla 300'den 350'ye, sonra 400 KB gzip'e yükseltildi (24 Eylül, ADR 0005 “Değişiklikler”); işlev adları pakette kalır. Sınıra 5 KB kaldı. SVG düzenleyicisinin geometrisi (§3 madde 7) ayrı ve ilk kullanımda yüklenen bir pakete gider, bu sınırı etkilemez. Her çekirdek diliminden sonra boyut ADR 0008'e yazılır.
- **Son doğrulama** (`main`, stil derleyicisi, 24 Eylül, bulut konteyneri): `pnpm typecheck` temiz; `pnpm test` 719 test geçti, 12 atlandı (fixture kaydedicileri, istekle çalışan ölçümler); `pnpm build` başarılı; `pnpm rust:test` temiz (324 test, clippy `-D warnings`; veritabanı testleri sır olmadığı için atlandı); `cargo fmt --all -- --check` temiz; stil derleyicisinin derin koşusu (20 000 sembol, 20 000 katman) TS'le aynı; `pnpm e2e` 134 denetim geçti, düşen üçü bu konteynerde düşen WebGPU denetimleri (§5; aynı piksel sayılarıyla). Pafta, MPYY ve temel sembol katalogları ile stil yöneticisinin ekran görüntüleri eski TS ile piksel piksel aynı. Önceki doğrulama (ifade dili): 725 test, 310 Rust testi, aynı e2e sonucu. DXF ölçüsündeki doğrulamada alt ajan ayrıca örnek bir dosyayı ezdxf 1.4 ile denetledi (hata yok); ölçülü dosya da hatasız ve düzeltmesiz okunur, ezdxf'in yeniden çizdiği değerler KentOS'unkiyle aynıdır. Tam takımda `ellipse.test.ts`'in yoğun örnekleme denetimi makine yüklüyken bir kez 5 sn sınırını aştı; kendi süresi 30 sn yapıldı (`67c663d`).
- **Ölçüm:**
...
7. **style-core:** kullanıcı 24 Eylül'de başlattı (“5 maddeyi de yap”), dilim dilim aynı yöntemle (§4).
   ...
   - **Y2 + Y3 stil derleyicisi yapıldı** (ADR 0008 “Stil derleyicisi”): yerleştirme geometrisi (işaret yerleri, gruplar, dalgalar, iç nokta), sembol × geometri → çizim ilkelleri, işleyicilerin çözümü ve GPU toplulukları `style-core/src/style`'da; katman tek çağrıda kurulur. Bekçi `style/compile.ts`, `style/geometry.ts`, `style/primitives.ts` ve `render/styledLayer.ts`'i de denetler. Başlangıç sınırı için karar alındı (400 KB, §6 madde 14).
...
14. ~~Başlangıç WASM sınırı (350 KB gzip) stil motorunun taşınmasıyla aşılacak.~~ **Kararlaştırıldı (24 Eylül):** kullanıcı sınırı 400 KB gzip'e yükseltmeyi seçti; işlev adları pakette kalır (ADR 0005 “Değişiklikler”). Stil derleyicisiyle paket 394,9 KB.

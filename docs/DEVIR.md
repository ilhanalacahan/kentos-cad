...
- **Son doğrulama** (`main`, sağlam kararlar R2b'den sonra, 24 Eylül, bulut konteyneri): `pnpm typecheck` temiz; `pnpm test` 711 test geçti, 6 atlandı (fixture kaydedicileri); `pnpm build` başarılı; `pnpm rust:test` temiz (255 test, clippy `-D warnings`; veritabanı testleri sır olmadığı için atlandı); `cargo fmt --all -- --check` temiz; `pnpm e2e` 109 denetim geçti, düşen üçü bu konteynerde düşen WebGPU denetimleri (§5; şeritten önceki `6505a6e` de aynı piksel sayılarıyla düşüyor). Tam takımda `ellipse.test.ts`'in yoğun örnekleme denetimi makine yüklüyken bir kez 5 sn sınırını aştı; kendi süresi 30 sn yapıldı (`67c663d`).
- **Ölçüm:**

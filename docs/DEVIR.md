...
- **WASM paketi:** 894 506 bayt, gzip 302 107 bayt (sağlam kararlar R1–R2b ve paketli dönüşümle; önce 883 626 / 297 902). Başlangıç sınırı kullanıcı kararıyla 300'den 350 KB gzip'e yükseltildi (24 Eylül, ADR 0005 “Değişiklikler”); işlev adları pakette kalır. Her çekirdek diliminden sonra boyut ADR 0008'e yazılır.
- **Son doğrulama** (`main`, sağlam kararlar R2b, Katmanlar paneli ve paketli dönüşüm, 24 Eylül, bulut konteyneri): `pnpm typecheck` temiz; `pnpm test` 717 test geçti, 7 atlandı (fixture kaydedicileri ve istekle çalışan dönüşüm ölçümü); `pnpm build` başarılı; `pnpm rust:test` temiz (257 test, clippy `-D warnings`; veritabanı testleri sır olmadığı için atlandı); `cargo fmt --all -- --check` temiz; `pnpm e2e` 119 denetim geçti, düşen üçü bu konteynerde düşen WebGPU denetimleri (§5; şeritten önceki `6505a6e` de aynı piksel sayılarıyla düşüyor). Tam takımda `ellipse.test.ts`'in yoğun örnekleme denetimi makine yüklüyken bir kez 5 sn sınırını aştı; kendi süresi 30 sn yapıldı (`67c663d`).
- **Ölçüm:**

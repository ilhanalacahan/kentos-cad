...
import type { SnapHit, SnapKind } from '../../../viewport/picking';

/**
 * The TypeScript PickIndex as it was before the Rust geometry store took its
 * place (docs/adr/0008, S1): the parity test (src/wasm/parity/store.test.ts)
 * holds the store to it until the TypeScript geometry is deleted (S3), when
 * the frozen store fixtures take over. Unchanged but for the imports and
 * the class name.
 */

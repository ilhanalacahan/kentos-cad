...
| `wasm/parity/store.test.ts` | Geometri deposu ile eski TS `PickIndex`'in, üst katman kararlarının ve araç önizlemelerinin (`parity/reference/picking.ts`, `overlay.ts`, `tools.ts`) yan yana karşılaştırması: örnek projede ve gizli/kilitli/yalnız kenar/ağaçta olmayan, etiket stilli katmanlı rastgele sahnede aynı imleç, tolerans, kenet türü, kutu, görünüm ve ölçekle seçme, kenar seçme (süzgeçli), kenet, pencere ve kesişim seçimi, çevreleyen şekil, çakışanlar, sınır kenarları, etiketler (her yerleşim, yazı, ölçü, düzenlenen), tutamaçlar, buda ve uzat (görünen ya da seçilen sınırlarla), dönüşüm ve esnet hayaletleri, seçim toplamları; turlar arasında düzenleme, geri alma, dış değişiklik, katman durumu ve yeniden yükleme, depo sırasının belgeninkiyle aynılığı |
...
src/
  ...
  wasm/                      Rust çekirdeğinin tarayıcı cephesi: core.ts (başlatma, op() çağrıları, NaN/±∞ geri çevirme, hata bildirimi, tipli toplu girişler: `triangulateMany`, geometri deposu `CoreStore`), pack.ts (nesneleri depo için sayı akışına paketleme), testSetup.ts (Vitest), parity/ (TS ↔ Rust çağrı kümeleri, karşılaştırma; reference/ eski TS PickIndex, üst katman ve araç önizlemesi hesapları), golden, çağrı ve depo fixture testleri; pkg/ `pnpm wasm` ile üretilir, depoya girmez
crates/

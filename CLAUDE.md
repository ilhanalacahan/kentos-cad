...
- Rust çekirdeği (`crates/geometry-core`) `model/geom`, `model/ops`, `model/geometry.ts`, `model/entities.ts` ve `render/triangulate.ts` işlemlerinin hepsini taşıyor (P0–P8: TS ile 20 000 rastgele durumda aynı sonuç, fixture'lar native ve WASM'da geçiyor). Uygulama çekirdeği açılışta yükler ve çağrı yolu kuruludur (ADR 0008). Seçme, kenet, etiket ve tutamaç kararları, araç önizlemeleri (S1) ve katman kurulurken çizilen geometri ile dolgu üçgenlemesi (S2) çekirdekten gelir; araçların ve işlemlerin öbür hesapları henüz TypeScript'tir. Bütün çağıranlar çekirdeğe geçince TS algoritması silinecek (S3). Yaylı nesnelerin sınır kutusu TS'de 72 parçalı ana hatla yaklaşık bulunuyor ve golden setinde yok (ADR 0002).
...
src/
  ...
  wasm/                      Rust çekirdeğinin tarayıcı cephesi: core.ts (başlatma, op() çağrıları, NaN/±∞ geri çevirme, hata bildirimi, tipli toplu girişler: `triangulateMany`, geometri deposu `CoreStore`), pack.ts (nesneleri depo için sayı akışına paketleme), testSetup.ts (Vitest), parity/ (TS ↔ Rust çağrı kümeleri, karşılaştırma; reference/ eski TS PickIndex, üst katman, araç önizlemesi ve katman geometrisi hesapları), golden, çağrı ve depo fixture testleri; pkg/ `pnpm wasm` ile üretilir, depoya girmez
crates/

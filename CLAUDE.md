...
| `wasm/parity/store.test.ts` | Geometri deposu ile eski TS `PickIndex`'in ve üst katman kararlarının (`parity/reference/picking.ts`, `overlay.ts`) yan yana karşılaştırması: örnek projede ve gizli/kilitli/yalnız kenar/ağaçta olmayan, etiket stilli katmanlı rastgele sahnede aynı imleç, tolerans, kenet türü, kutu, görünüm ve ölçekle seçme, kenar seçme (süzgeçli), kenet, pencere ve kesişim seçimi, çevreleyen şekil, çakışanlar, sınır kenarları, etiketler (her yerleşim, yazı, ölçü, düzenlenen) ve tutamaçlar; turlar arasında düzenleme, geri alma, dış değişiklik, katman durumu ve yeniden yükleme, depo sırasının belgeninkiyle aynılığı |

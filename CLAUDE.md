...
**Hesap Rust çekirdeğindedir** (`crates/geometry-core`, ADR 0008). `model/ops`'un bütün dosyaları ve üst düzey `model/geom` modülleri (bindirme ve alan cebiri, paralel, ölçmecilik, şekiller, teğet daire, öteleme, tarama, ölçü) ince cephelerdir: TS dosyası adı, tipi ve belge yorumunu tutar, işlevi `op('ad')` ile çekirdeğe gider. Nesne döndüren işlemler `ops/entityOp.ts` ile sarılır (çekirdeğin yazmadığı `bulges`/`holes`, `doc.update` birleştirmesinde eski yayları bırakmasın diye `undefined` olarak eklenir). Aşağıdaki tablo işlemlerin ne yaptığını anlatır; Rust'taki karşılıkları aynı adlı dosyalardadır. İlkel modüller (`geometry.ts`, `affine`, `arc`, `bulge`, `intersect`, `ellipse`, `spline`, `entities.ts`) çevrilene kadar (S3b) TS'tedir.
...
- Rust çekirdeği (`crates/geometry-core`) `model/geom`, `model/ops`, `model/geometry.ts`, `model/entities.ts` ve `render/triangulate.ts` işlemlerinin hepsini taşıyor (P0–P8: TS ile 20 000 rastgele durumda aynı sonuç, fixture'lar native ve WASM'da geçiyor). Uygulama çekirdeği açılışta yükler ve çağrı yolu kuruludur (ADR 0008). Seçme, kenet, etiket ve tutamaç kararları, araç önizlemeleri (S1), katman kurulurken çizilen geometri ile dolgu üçgenlemesi (S2), `model/ops` ve üst düzey `model/geom` işlemleri (S3a; TS algoritması silindi), işlem araçlarının geometrisi (S4), araçların ve nesne izlemenin satır içi hesapları (S5) çekirdekten gelir. İlkel modüller (`geometry.ts`, `affine`, `arc`, `bulge`, `intersect`, `ellipse`, `spline`, `entities.ts`, `render/triangulate.ts`) henüz TypeScript'tir (S3b). Büyük seçimi taşımak, kopyalamak ve yapıştırmak tek çağrıdır ama JSON'dan geçer: 10 000 nesnede ~0,15 s (TS'te ~0,01 s); dönüşüm depoda yapılıp paketli döndürülecek. Yaylı nesnelerin sınır kutusu TS'de 72 parçalı ana hatla yaklaşık bulunuyor ve golden setinde yok (ADR 0002).
...
src/
  ...
  model/                     Belge, varlıklar, geometri, katmanlar, seçim, proje ayarları, örnek proje, standart katman ağacı (standardLayers), boş yeni proje (newProject)
    ...
    geom/                    Geometri çekirdeği: afin, yay, bulge, kesişim (TS, S3b'ye kadar); öteleme, teğet daire, düzlem bindirme ve alan cebiri, şekiller, ölçmecilik, tarama, ölçü (Rust cepheleri) (+ testler)
  ...
  wasm/                      Rust çekirdeğinin tarayıcı cephesi: core.ts (başlatma, op() çağrıları, NaN/±∞ geri çevirme, hata bildirimi, tipli toplu girişler: `triangulateMany`, `offsetPathXY`, `hatchLinesXY`, `cornerTexts`; durum tutan sınıflar: geometri deposu `CoreStore`, yüz dizini `CoreFaceIndex`), pack.ts (nesneleri depo için sayı akışına paketleme), testSetup.ts (Vitest), parity/ (TS ↔ Rust çağrı kümeleri, karşılaştırma; reference/ eski TS PickIndex, üst katman, araç önizlemesi ve katman geometrisi hesapları, işlem araçlarının hesapları, nokta girişi, nesne izleme ve araçların satır içi hesapları), golden, çağrı ve depo fixture testleri; pkg/ `pnpm wasm` ile üretilir, depoya girmez
crates/
  ...
  wasm/                      Çekirdeğin tarayıcı sınırı (wasm-bindgen): çağrı tablosu (opId/callOp, JSON), düz Float64Array girişleri, geometri deposu (store.rs) ve yüz dizini (faces.rs)

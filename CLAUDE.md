...
**`io/`** (dosya alışverişi: biçim worker'ı `formatsWorker.ts`, sayfa tarafı `client.ts`, okunan nesneleri belgeye koyan `apply.ts`, koordinat listesi seçenekleri `coords.ts`; `pkg/` üretilir) `processing` ile aynı düzeydedir: core, geo, model ve contracts'ı içe aktarır, DOM'a dokunmaz; pencereleri `ui/io/`'dadır. Biçimlerin kendisi Rust'tadır (`crates/formats`, §9.7); uygulamanın da hesapladığı geometriyi (yaylı halkanın noktaları, alan, içerme) ortak çekirdekten alır. `io/` ve `ui/io/` başlangıç paketine girmez, komut çalışınca yüklenir (§20).
...
| `wasm/parity/parity.test.ts` | TypeScript ile Rust çekirdeğinin yan yana karşılaştırması: her çağrı kümesinin adlı sınır durumları ve işlem başına 200 tohumlu rastgele çağrı (`PARITY_CASES` artırır); sayılar golden toleransıyla, metin, uzunluk ve anahtarlar tam. Yalnız TS'i duran işlemler; çevrilenler donmuş fixture'larla sınanır (ADR 0008) |
| `wasm/parity/store.test.ts` | Geometri deposu ile eski TS `PickIndex`'in, üst katman kararlarının, araç önizlemelerinin ve katman kurucularının geometrisinin (`parity/reference/picking.ts`, `overlay.ts`, `tools.ts`, `draw.ts`) yan yana karşılaştırması: örnek projede ve gizli/kilitli/yalnız kenar/ağaçta olmayan, etiket stilli katmanlı rastgele sahnede aynı imleç, tolerans, kenet türü, kutu, görünüm ve ölçekle seçme, kenar seçme (süzgeçli), kenet, pencere ve kesişim seçimi, çevreleyen şekil, çakışanlar, sınır kenarları, etiketler (her yerleşim, yazı, ölçü, düzenlenen), tutamaçlar, buda ve uzat (görünen ya da seçilen sınırlarla), dönüşüm ve esnet hayaletleri, seçim toplamları, çizilen geometri (stil ve vurgu, kırpmalı ve kırpmasız, tek nesnelik `styledGeometry` dahil) ve ifadelerin geometri değerleri; turlar arasında düzenleme, geri alma, dış değişiklik, katman durumu ve yeniden yükleme, depo sırasının belgeninkiyle aynılığı |
| `wasm/store.wasm.test.ts` | Dondurulmuş depo fixture'ları (`fixtures/geometry/v1/store-v1.json`, işlem araçlarının `store-processing.json`'u) WASM deposundan; işlem sorguları uygulamanın yolundan (`ObjectStore`) |
| `wasm/parity/processing.test.ts` | İşlem araçlarının çekirdekteki geometrisi ile eski TS'in (`parity/reference/processing.ts`) yan yana karşılaştırması: örnek projede ve düzenlenen rastgele sahnede her yerleşik aracın rastgele değerlerle sayfada ve worker'ın iş işleyicisinde çalışması (sayfa ile worker bit bit aynı), görünen kapsam (görünümün ve kendi deposundan), penceredeki ifade önizlemesi |
...
- `crates/wasm`: art arda dizilmiş halkaların aralıkları (veriden büyük boyutlar kırpılır).
- `crates/geometry-core` `geom/arrangement.rs`, `ops/transform.rs`: sarım dizininin açı toplamıyla aynı sonucu (yay ucundan geçen ışın dahil), toplu dönüşümün sırası.
...
- `crates/geometry-core` `tools`: kutupsal dizinin dönmeyen kopyalarının TM'de aynı ötelemesi, çekilen boyun adımı, izlemede “tam üstü”nün aynı X'i, hizaların eksenlerle birleşmesi, serbest imlecin `tracking: null` yazılması.
...
- `crates/application/tests/identity.rs` (geçici veritabanı): tenant ayrımı (satır güvenliği, başka tenant adına yazma), sunucu rolünün yapamadıkları (proje satırı silmek dahil), yerel giriş, oturumlar, koltuk ve üyelik, OpenID kimliği.
...
- Rust çekirdeği (`crates/geometry-core`) `model/geom`, `model/ops`, `model/geometry.ts`, `model/entities.ts` ve `render/triangulate.ts` işlemlerinin hepsini taşıyor (P0–P8: TS ile 20 000 rastgele durumda aynı sonuç, fixture'lar native ve WASM'da geçiyor). Uygulama çekirdeği açılışta yükler ve çağrı yolu kuruludur (ADR 0008). Seçme, kenet, etiket ve tutamaç kararları, araç önizlemeleri (S1), katman kurulurken çizilen geometri ile dolgu üçgenlemesi (S2), `model/ops` ve üst düzey `model/geom` işlemleri (S3a; TS algoritması silindi), işlem araçlarının geometrisi (S4), araçların ve nesne izlemenin satır içi hesapları (S5) çekirdekten gelir. S3b'de ilkel modüller (`geometry.ts`'in ölçüleri, `affine`, `arc`, `bulge`, `intersect`, `ellipse`, `spline`, `entities.ts`, `render/triangulate.ts`) de cepheye döndü; stil motoru, ifade dili ve SVG düzenleyicisinin kendi geometrisi dışında TS'te geometri algoritması kalmadı. Büyük seçimi taşımak, kopyalamak ve yapıştırmak tek çağrıdır ama JSON'dan geçer: 10 000 nesnede ~0,15 s (TS'te ~0,01 s); dönüşüm depoda yapılıp paketli döndürülecek. Yaylı nesnelerin sınır kutusu 72 parçalı ana hatla yaklaşık bulunuyor ve golden setinde yok (ADR 0002).
...
src/
  ...
  model/                     Belge, varlıklar, geometri, katmanlar, seçim, proje ayarları, örnek proje, standart katman ağacı (standardLayers), boş yeni proje (newProject)
    expression/              İfade dili (ayrıştırma, derleme, değerler, işlevler; işlem araçları ve stil motoru kullanır)
    geom/                    Geometri çekirdeğinin cepheleri (Rust'ta): afin, yay, bulge, kesişim, elips, eğri; öteleme, teğet daire, düzlem bindirme ve alan cebiri, şekiller, ölçmecilik, tarama, ölçü (Rust cepheleri) (+ testler)
  ...
  wasm/                      Rust çekirdeğinin tarayıcı cephesi: core.ts (başlatma, op() çağrıları, NaN/±∞ geri çevirme, hata bildirimi, tipli toplu girişler: `triangulateMany`, `offsetPathXY`, `hatchLinesXY`, `cornerTexts`; sayı alan küçük ölçüler (`coreDist` …) ve kazıma tamponlu halka ölçüleri (`ringCentroid` …); durum tutan sınıflar: geometri deposu `CoreStore`, yüz dizini `CoreFaceIndex`), pack.ts (nesneleri depo için sayı akışına paketleme), testSetup.ts (Vitest), parity/ (TS ↔ Rust çağrı kümeleri, karşılaştırma; reference/ eski TS PickIndex, üst katman, araç önizlemesi ve katman geometrisi hesapları, işlem araçlarının hesapları, nokta girişi, nesne izleme ve araçların satır içi hesapları), golden, çağrı ve depo fixture testleri; pkg/ `pnpm wasm` ile üretilir, depoya girmez
crates/
  ...
  wasm/                      Çekirdeğin tarayıcı sınırı (wasm-bindgen): çağrı tablosu (opId/callOp, JSON), düz Float64Array ve sayı girişleri, halka ölçüleri için kazıma tamponu, geometri deposu (store.rs) ve yüz dizini (faces.rs)
...
apps/api/                    kentosd: `serve` (Axum; http/ auth, projects, ws, error; oidc.rs; hub.rs; saatlik olay budaması), `db-setup`, `migrate`, yönetim komutları (cli.rs), config.rs

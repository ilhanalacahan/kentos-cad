...
1. **CAD çekirdeği:**
   ...
   - **Netcad çizim eşdeğerliği (referans Netcad'dir; AutoCAD ikincil ölçüttür. Bağlayıcı hedef: çizim kusursuz olmadan başka işe geçilmez):**
     - *Netcad'e özgü, var:* Koordinat hesap makinası (yan nokta, kenar kesişimi, doğru kesişimi, hat üzerinde nokta, açı-mesafe, orta nokta), köşe yuvarla/kır/sil, paralel al (ötele), böl, birleştir; **alan işlemleri** (birleştir, kesiştir, çıkar, böl; yaylar ve ortak sınırlar tam), adalı alan, alana çevir, içine tıklayarak alan (adalarla), çizgiye çevir.
     - *Netcad'e özgü, eksik:* Paralel Çizgi (eksen çizilirken sol/sağ genişlikte paralel hatlar, köşeler kendiliğinden birleşir); alanı verilen alana göre bölme (ifraz, topolojiyle); Dik in / Dik çık çizim komutları; sembol ve blok yerleştirme; klotoid (spiral) nesnesi; poligon ve kutupsal ölçü hesapları (Hesap menüsü).
...
- Tarama ilişkisel değil: sınır değişince tarama güncellenmez. Adaları yalnızca adalı alandan alır; çizgilerin kapattığı bölgeye ve içindeki başka nesnelere göre ada algılama (AutoCAD HATCH gibi) `faceIndex` ile yapılacak.
